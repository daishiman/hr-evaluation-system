...
見た目(色・組版・部品・状態・モーション)の規律は Skill `jp-web-design` を参照。「AIっぽい」指摘時の対応は本書冒頭(frontmatter)の通り、まず Skill `design-judgment` を使う。
