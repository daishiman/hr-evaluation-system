...
| KPI基準定義_配点（保留） | 2〜166行目 | 165 | `scheme_items` / `scheme_rank_ratios` | 8 / 5 | −152 | 配点は「項目×ランク」の165マスだが、評価セットは8項目・ランク比率は5段階。165マスは「8項目 × 5ランク比率」に整理して持ち替えた（§6-1） |
...
| KPI基準定義_配点（保留）の165マスのうち、評価セットに入らない項目の行 | 157 | 配点は「毎回8項目・100点」で組む仕組みにしたため、選ばれた8項目分だけが `scheme_items` になる。**残りの項目の配点も消してはいない**（`data/kpi-points.json` に全165行が残っていて、項目を入れ替えれば読み直される） |
