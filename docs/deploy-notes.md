...
見る順番は「画面で発行した鍵 → サーバーの設定値」です。先に画面の鍵を見るのは、そちらが画面から即座に止められる唯一の鍵だからです（設定値を止めるにはターミナルが要ります）。どちらか一方に合えば通り、**画面の鍵を失効させても設定値が残っていれば払い出しは止まりません**。完全に止めるときは両方を消します。32文字未満の設定値は未設定と同じ扱いです。
...
4. 漏れたと思ったら「鍵を作り直す」を押す。古い鍵はその時点で使えなくなる
5. 受け取りを止めるときは「鍵を失効させる」を押す
