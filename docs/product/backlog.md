...
| UX-025 | observe | どの配色が選ばれたかの集計は `/api/theme-choice` の GET でしか読めず、画面が無い | 票が貯まり、標準の配色を決める判断に入る | 票が数十件を超えてから、管理画面に1枚足すか、その場の書き出しで足りるかを決める。まず数え始めることを優先した | [実装](../../src/app/api/theme-choice/route.ts) / [見本](./theme-gallery.md) |
| UX-026 | observe | 票は「切り替えた回数」であり「使い続けている人数」ではない。5系統を順に試すと5票入るため、試し押しの多い配色ほど数が伸びる | 票が貯まり、人気の配色を標準に採用する判断に入る | 誰が選んだかを保存しない方針（[仕様 §配色](./spec.md)）を保つため、端末ごとに1票へ丸めるか、一定時間そのままだった組み合わせだけを数えるかを決める。まず素の回数で傾向を見る | [実装](../../src/lib/theme-usage.ts) |
