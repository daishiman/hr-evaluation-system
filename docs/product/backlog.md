...
| FEATURE-025 | observe | 似ている記録票は本文に並べるだけで、重複の統合・自動closeはしない（同じ画面・同じ種類の直近3件） | 同じ不具合の記録票が繰り返し立ち、突き合わせの手間が申告される | GitHub 側の重複検索を足すか、起票前に「これと同じですか」と確認する画面を挟むかを決める | [仕様 §26-8](./spec.md) / [起票の契約](../../system-spec/improvement-requests.md) |
...
| FEATURE-028 | observe | 一括送信は1件ずつ順番に送るため、100件を超えると待ち時間が長い（GitHub の回数制限を避けるため意図的に順次にしている） | 1回の送信で100件を超える運用が定着する | 実測してから、待ち行列に載せて画面を閉じても続く形にするかを決める | [仕様 §26-9](./spec.md) / [一括同期の契約](../../system-spec/improvement-requests.md) |
| FEATURE-029 | observe | 送信中に画面を閉じると、そこから先の行は送られない（送信済みの分は確定する） | 送信の中断が繰り返し申告される | 中断した続きを次に開いたとき示すか、サーバー側で続きを引き受けるかを決める | [仕様 §26-9](./spec.md) / [画面](../../src/components/ImprovementBulkTable.tsx) |
| FEATURE-030 | observe | 「更新あり」の判定は要望の中身だけを見るため、記録票を GitHub 側で手直ししてもアプリ側は「変更なし」のまま | GitHub 側での手直しが上書きされたという申告が出る | 記録票側の更新時刻も併せて見るか、上書き前に差分を見せるかを決める | [仕様 §26-9](./spec.md) / [判定の正本](../../src/lib/domain/improvement-sync.ts) |
...
| FEATURE-032 | observe | GitHub 側の状態は「取り込む」を押したときだけ追従する（自動で見に行かない） | 記録票を GitHub 側で閉じる運用が定着し、アプリ側とのずれが常態化する | 一覧を開いたときにまとめて取り込むか、Webhook で受けるかを決める | [仕様 §26-10](./spec.md) / [落とす・戻すの契約](../../system-spec/improvement-requests.md) |
...
| SECURITY-012 | observe | 通信の中身は鍵の名前で伏せるため、氏名・評価コメント以外の無害な値まで `***` になり、原因の特定に届かないことがある | 記録票を見ても原因が分からず、開発ツールでの再取得を繰り返す事例が出る | 伏せた件数を数え、過剰な鍵名を実データで見直す。緩める場合は評価データ本文を除外できる形だけを採る | [伏せ方の正本](../../src/lib/domain/improvement-issue.ts) / [仕様 §26-7](./spec.md) |
...
| PERFORMANCE-004 | ready | 公開バンドルの圧縮後実測が 2,756.6 KiB（無料プラン3,072 KiBの89.7%。CI の本番ビルド実測）で警告域に入った（改善要望の撮影ライブラリは押したときだけ遅延読込。まとめ送り・廃棄の追加は API の道を増やさず既存の道へ同居させた） | 次の機能追加・依存更新前 | `wrangler deploy --dry-run` の構成内訳を比較し、未使用・重複コードを優先して3072 KiBまでの余白を戻す | [容量チェック](../../scripts/check-bundle-size.mjs) / [デプロイ注意](../deploy-notes.md) |
