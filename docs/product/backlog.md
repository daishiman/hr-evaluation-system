...
| FEATURE-035 | observe | 払い出し用の鍵は1本だけで、失効させると作業する側が全員止まる | 作業する側が複数に増える、または鍵の入れ替えを定期的に行う運用になる | 鍵を複数本受け付けるか、期限付きにするかを決める | [鍵の判定](../../src/lib/domain/agent-api.ts) / [デプロイ注意 §5](../deploy-notes.md) |
