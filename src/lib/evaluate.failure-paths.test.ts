...
describe("計算式そのものが手に負えない場合", () => {
  /**
   * 括弧が極端に深い式は、計算式の書式としては正しいのに、計算の途中で
   * 処理そのものが続けられなくなる（＝計算式の不備を表す例外ではない形で止まる）。
   * 「実際には起きない」と決めつけず、本当に起きることをここで確かめている。
   */
  it("その項目だけを判定外にし、ほかの項目とほかの人の集計は続く", async () => {
    await seedCompany(current);
    // 括弧を2万重ねた式。書式としては正しいが、計算を最後まで進められない。
