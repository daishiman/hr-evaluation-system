...
- **導入前に §0 の判断を行う**。導入しないほうがよい状況(まだ公開していない・テストが1本もない)であれば、その旨を伝えて中止する。作ることが目的ではない。
...
- 既に `.github/workflows/` に何かある場合は**上書きしない**。中身を読み、差分と重複(特に Cloudflare Workers Builds との二重デプロイ)を報告してから判断を仰ぐ。
