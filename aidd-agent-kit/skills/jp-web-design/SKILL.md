...
0. **順序が結果を決める — 表から書き始めない**。場面の1文 → ラベル剥がし → **伝わらないものだけ最小限に補う** → グループ化 → 優先順位 → 表示用加工 → レイアウト形式の選定 → 機能と意味づけの装飾。設計とデザインを分けず、この8工程で作る(`references/information-design.md`)。**装飾を後から足しても画面は良くならない。**
